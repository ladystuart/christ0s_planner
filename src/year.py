...
from config.utils import add_source_label_second_level as add_source_label_year, add_icon_image
...
class Year(tk.Frame):
    ...
    def __init__(self, parent, main_window, year):
        ...
        self.icon_image_original = asyncio.run(add_icon_image(ICONS_PATHS['year']))

        self.clickable_label = add_source_label_year(
            self,  # Parent element
    ...
    def on_button_click(self, label):
        ...
        if label == PAGES_NAMES['calendar']:
            calendar_frame = Calendar(self.parent, main_window=self.main_window, year=self.year)
            calendar_frame.pack(fill=tk.BOTH, expand=True)

        elif label == PAGES_NAMES['yearly_plans_inner']:
            yearly_plans_inner_frame = YearlyPlansInner(self.parent, main_window=self.main_window, year=self.year)
            yearly_plans_inner_frame.pack(fill=tk.BOTH, expand=True)

        elif label == PAGES_NAMES['habit_tracker']:
            habit_tracker_frame = HabitTracker(self.parent, main_window=self.main_window, year=self.year)
            habit_tracker_frame.pack(fill=tk.BOTH, expand=True)

        elif label == PAGES_NAMES['gratitude_diary']:
            gratitude_diary_frame = GratitudeDiary(self.parent, main_window=self.main_window, year=self.year)
            gratitude_diary_frame.pack(fill=tk.BOTH, expand=True)

        elif label == PAGES_NAMES['best_in_months']:
            best_in_months_frame = BestInMonths(self.parent, main_window=self.main_window, year=self.year)
            ...
            review_frame = Review(self.parent, main_window=self.main_window, year=self.year)
    ...
    def add_quote_and_image(self):
        ...
        for label, icon_path in button_data:
            ...
            # Resize the image to fit the button (equivalent to subsampling)
            self.icon_image_resized = self.icon_image_original.resize(
                (self.icon_image_original.width // 2, self.icon_image_original.height // 2), Image.Resampling.LANCZOS)

            # Convert the image for use with Tkinter
            button_image = ImageTk.PhotoImage(self.icon_image_resized)
