...
from config.utils import add_icon_image
...
def add_image_to_button(button, image_path, width, height):
    ...
    """
    try:
        image = asyncio.run(add_icon_image(image_path))
        ...
        print(f"Error image GET {image_path}: {e}")
...
class ListsForLife(tk.Frame):
    ...
    def add_buttons(self, items):
        ...
        def create_button(index, item):
            ...
            button = tk.Button(
                ...
                state=tk.DISABLED  # Initially disable the button (to make it invisible)
            )
            ...
            self.after(300, lambda: enable_button(button))

        def enable_button(button):
            """
            Activates the given button and increments the counter of loaded buttons.
            Once all 4 buttons are loaded, it calls the enable_buttons method of the main window.
            ...
            self.loaded_buttons_count += 1

            if self.loaded_buttons_count == 4:
                self.main_window.enable_buttons()

        # Create buttons for the first 4 items in a thread
        for index, item in enumerate(items[:4]):  # Only use the first 4 items
            delay = index * 50  # Define delay for each button (50 ms delay between each)
            create_button(index, item)
    ...
    def on_button_click(self, item):
        ...
        if item["title"] == "Reading":
            ...
            goals_frame = Reading(self.parent, main_window=self.main_window)
            goals_frame.pack(fill=tk.BOTH, expand=True)
        elif item["title"] == "Wishlist":
            from src.wishlist import Wishlist
            goals_frame = Wishlist(self.parent, main_window=self.main_window)
            goals_frame.pack(fill=tk.BOTH, expand=True)
        elif item["title"] == "Goals":
            from src.goals import Goals
            goals_frame = Goals(self.parent, main_window=self.main_window)
            goals_frame.pack(fill=tk.BOTH, expand=True)
        elif item["title"] == "Courses":
            from src.courses import Courses
            goals_frame = Courses(self.parent, main_window=self.main_window)
            goals_frame.pack(fill=tk.BOTH, expand=True)
