...
async def load_tasks_from_server():
    """
    Asynchronously loads tasks (goals) from the server.

    This function sends an asynchronous HTTP GET request to the server's `/get_goals` endpoint.
    ...
    Returns:
        list: A list of tasks (goals) from the server, or an empty list if an error occurs.
    """
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{SERVER_URL}/get_goals", ssl=SSL_ENABLED) as response:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                    f"{SERVER_URL}/update_goal_status",
...
async def remove_task(task, frame, tasks):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{SERVER_URL}/delete_goal", json={"title": task["text"]},
...
async def save_task_changes(task, new_text, edit_window, frame, tasks):
    ...
    if any(existing_task["text"] == new_text for existing_task in tasks):
        messagebox.showerror("Error", "Goal with this title already exists!")
        edit_window.lift()
        return tasks
    ...
    if new_text.strip():
        ...
        try:
            async with aiohttp.ClientSession() as session:
                url = f"{SERVER_URL}/update_goal_title"
...
async def add_goal(task):
    """
    Save a new goal to the PostgreSQL database.
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{SERVER_URL}/add_new_goal", json={"title": task['text']}, ssl=SSL_ENABLED) as response:
                if response.status == 200:
                    return await response.json()
                else:
...
class Goals(tk.Frame):
    ...
    def __init__(self, parent, main_window):
        ...
        2. Stores the parent and main application window reference.
        3. Loads the goal data from server and initializes `self.tasks`.
    ...
    def edit_task(self, task, frame):
        ...
        6. Updates the task in the parent frame and in the underlying data structure by calling `save_task_changes`.
        """
    ...
    def add_task(self):
        ...
        if task_text:
            ...
            if any(task["text"] == task_text for task in self.tasks):
                messagebox.showerror("Error", "Goal with the same title already exists!")
                return

            new_task = {'text': task_text, 'completed': False}  # Create new goal
            self.tasks.append(new_task)
            asyncio.run(add_goal(new_task))  # Save to database
