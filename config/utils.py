...
import webbrowser
import json
from config.settings import INTERFACE, USEFUL_LINKS
...
def add_source_label(parent, link, title, bg_color, font):
    ...
    try:
        icon_image_original = Image.open(link)
        icon_image = icon_image_original.resize((INTERFACE['source_icon_dimensions'],
                                                 INTERFACE['source_icon_dimensions']), Image.Resampling.LANCZOS)
...
def add_icon_and_label(parent, text, icon_path, bg_color):
    ...
    try:
        ...
        icon_image = icon_image_original.resize((INTERFACE['icon_dimensions'],
                                                 INTERFACE['icon_dimensions']), Image.Resampling.LANCZOS)
        icon_photo = ImageTk.PhotoImage(icon_image)
        icon_image_original.close()
...
def add_title_icon(header_frame, icon_width, icon_height, row, column, icon):
    ...
    try:
        ...
        left_icon_resized = left_icon_image.resize((icon_width, icon_height),
...
def add_source_label_second_level(parent, icon_path_1, clickable_text, click_command, icon_path_2, text):
    ...
    create_clickable_text(header_frame, clickable_text, command=click_command)
...
def add_source_label_third_level(parent, icon_path_1, year_name, clickable_text, click_command_1, icon_path_2,
    ...
    create_clickable_text(header_frame, clickable_text, command=click_command_1)

    create_icon_label(header_frame, icon_path_2)

    create_clickable_text(header_frame, year_name, command=click_command_2)
...
def create_icon_label(frame, icon_path, icon_size=(INTERFACE['source_icon_dimensions'],
    ...
    """
    Creates an icon label inside a given frame by loading an image from the provided path,
    ...
    The function uses the `Pillow` library to handle image loading and resizing, and
    integrates the image into a `tkinter.Label` widget.

    :param frame: The parent widget (usually a frame or window) where the icon label will be placed.
    :type frame: tkinter.Widget
    :param icon_path: The file path to the icon image that will be displayed.
    :type icon_path: str
    ...
    try:
        ...
        print(f"Icon load error {icon_path}: {e}")
...
def load_tasks_from_json(json_file):
    ...
    This method attempts to read the specified JSON file containing a list of tasks.
    If the file exists and is correctly formatted, it loads the tasks into the program.
    If the file does not exist or is corrupted, it handles the error gracefully and returns an empty dictionary.

    :param json_file: The path to the JSON file that contains the task data.
    :return: A dictionary containing the loaded tasks. If an error occurs, an empty dictionary is returned.
    """
    try:
